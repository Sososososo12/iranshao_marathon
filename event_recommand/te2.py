...
from event_recommand import email attention
...
for stopnum in range(20000,user_id_len,5000):
    ...
    #     'name':name
    # })
    filename = './user_done_participant/test{}.xls'.format(str(stopnum))
    if os.path.exists(filename):
        print('已存在文件名：'+'test{}'.format(str(stopnum)))
    else:
        data.to_excel(u'./user_done_participant/test{}.xls'.format(str(stopnum)), index=False, encoding='"utf_8_sig')
        print('创建文件名：'+'test{}'.format(str(stopnum)))
print('已全部创建完成！')
